...
export const createDuel = async (challengerId, opponentId, challengerName, opponentName, lang, words) => {
  const payload = {
...
export const respondDuel = async (duelId, accept) => {
  const patch = { status: accept ? 'active' : 'declined', updated_at: new Date().toISOString() }
  const { data, error } = await supabase.from('duels').update(patch).eq('id', duelId).select('*').single()
  if (error) {
...
export const submitDuelScore = async (duelId, isChallenger, score, done) => {
  const { data: duel, error: readError } = await supabase.from('duels').select('*').eq('id', duelId).single()
  if (readError) {
  ...
  const { data, error } = await supabase.from('duels').update(next).eq('id', duelId).select('*').single()
  if (error) {
